#111217
#ruby barcoding_pipe.rb -m 111217/111217_tagPrimerMaps/tagPrimerMap_111217A.txt -m 111217/111217_tagPrimerMaps/tagPrimerMap_111217B.txt -m 111217/111217_tagPrimerMaps/tagPrimerMap_111217C.txt -m 111217/111217_tagPrimerMaps/tagPrimerMap_111217D.txt -w 'https://uni-bonn.sciebo.de/remote.php/webdav/NGS Rohdaten/111217 ABCD GBoL/Run_0382-4_D01.tar' -o 111217 -s 111217/111217_tagPrimerMaps/adapterset_platepools.fasta -j 111217

#111148
#ruby barcoding_pipe.rb -m 111148/111148_tagPrimerMaps/tagPrimerMap_111148A.txt -m 111148/111148_tagPrimerMaps/tagPrimerMap_111148B.txt -w 'https://uni-bonn.sciebo.de/remote.php/webdav/NGS Rohdaten/111148 AB GBoL Caryophyllales 1&2/1_A01.tar' -o 111148 -s 111148/111148_tagPrimerMaps/adapterset_platepools.fasta -j 111148

#111158
#ruby barcoding_pipe.rb -m 111158/111158_tagPrimerMaps/tagPrimerMap_111158A.txt -m 111158/111158_tagPrimerMaps/tagPrimerMap_111158B.txt -w 'https://uni-bonn.sciebo.de/remote.php/webdav/NGS Rohdaten/111158 AB GBoL Caryophyllales 3&4/3_G01.tar' -o 111158 -s 111158/111158_tagPrimerMaps/adapterset_platepools.fasta -j 111158


#test all arguments:
#ruby /data/data2/lara/Barcoding/barcoding_pipe.rb -m /data/data2/lara/Barcoding/111217_fasta_test/111217_tagPrimerMaps/tagPrimerMap_111217A.txt -w 'https://uni-bonn.sciebo.de/remote.php/webdav/NGS Rohdaten/111217 ABCD GBoL/Run_0382-4_D01.tar' -d 1234 -t Embryophyta -i 0.97 -q 2 -p 3 -b 4 -s /data/data2/lara/Barcoding/111217_fasta_test/111217_tagPrimerMaps/tagPrimerMap_111217A.txt -f /data/data2/lara/Barcoding/111217_fasta_test/111217_tagPrimerMaps/tagPrimerMap_111217A.txt -o /data/data2/lara/Barcoding/111217_fasta_test
...
begin	
	...
	class OptparseExample
		...
		def self.parse(args)
			...
			opt_parser = OptionParser.new do |opts|
				opts.banner = "Usage: exonerate_multifasta.rb [options]"
				...
				opts.on("-k", [:fasta, :fastq, :bam], "--require FORMAT",
					"Please provide the FORMAT of your read file.") do |kind|
					options.kind = kind
				end
				opts.on("-f", "--require FILE", "Please provide the read FILE.") do |file|
					options.file = file
	     		end
	     		opts.on("-j", "--require TITLE", "Please provide a job TITLE when executing the script") do |title|
					options.title = title
	     		end
				opts.on("-t", "--taxon [TAXA]", "Set taxon") do |taxon|
					taxa_translator = {"Embryophyta" => "Embryophyta", "Marchantiophytina" => "Marchantiophyta", "Anthocerotophytina" => "Anthocerotophyta", "Lycopodiophytina" => "Lycopodiopsida", 
						"Equisetophytina" => "Equisetidae", "Filicophytina" => "Polypodiopsida", "Magnoliopsida" => "Magnoliopsida"}
	...
	#test if flags work:
	#p $options.to_h
	#p $options.to_h.values.length #should be 11
	#abort

	if $options["map"].length > 1 && $options["tpm_splitter"] == nil
		p "Please provide tags for assigning the according tagPrimerMap."
		abort
	elsif $options["map"].length == 1 && $options["tpm_splitter"] != nil
		p "You did not provide multiple tagPrimerMaps. Please do so."
		abort
	end
	...
	def calculateMismatches(primer, allowed_mismatches)
		#wobbles_2, wobbles_3 = /[SWYRKM]/, /[BVDH]/
	...
	#TODO: Job title alternative
	job_title = $options["title"]
	#job_title = "#{$options["file"]}".split("/")[-1].split(".f")[0]
	...
	##########################################
	#QUALITYFILTERING / SEQUEL DATA PROCESSING
	##########################################
	...
	if $options["kind"] == "bam"
		...
		if $options["file"].include?("https")
			...
			`tar -xvf #{$options["file"]} -C #{webdav_folder} --strip-components 1`
		end
		...
		`ccs --minLength=100 --numThreads=#{calculateNumThreads()} --minPasses=2 --minReadScore=0.6 --minPredictedAccuracy=0.95 #{bam} #{ccs_file}`
		...
		tpms.each do |letter|
			...
			seqs_pre_filtering += `wc -l '#{fasta_folder}/#{job_title}#{letter}.fasta'`.to_i / 2
		end

		seqs_post_filtering_seqtk = "NA"

	elsif $options["kind"] == "fastq"
		...
		seqs_pre_filtering = `wc -l '#{$options["file"]}'`.to_i / 4
		seqs_post_filtering_seqtk += `grep -c "^>" '#{$options["output"]}/#{job_title}_intermediate_out/seqtk_filtered.fasta'`.to_i
	elsif $options["kind"] == "fasta"
		filtered_fasta << $options["kind"]
		seqs_pre_filtering = `wc -l '#{$options["file"]}'`.to_i / 2
		seqs_post_filtering_seqtk = "NA"
	end
	...
	regions_array.each do |region|
		...
		region_dir.each do |parsed_file|
			...
			if !File.exist?("#{blast_output_dir}/#{gbol}")
				p "blastn -num_threads #{calculateNumThreads()} -db nt -max_target_seqs 1 -outfmt '6 qseqid sskingdoms sscinames staxids evalue length pident nident mismatch gaps sacc sseqid stitle' -evalue 1e-10 -query '#{usearch_output_dir}/#{gbol}_centroid' -out '#{blast_output_dir}/#{gbol}'" 
	...
	regions_array.each do |region|
		...
		consensus_files.each do |consensus_file|
			gbol = consensus_file.scan(/\w+\d+/)[-1]
	...
	#File.open("#{$user_output_dir}/#{job_title}_stderr.txt", "w+") do |out|
